...
>  So the environement variable looks like this in the host:
> ``` bash
...
> ```
>  And the evaluation of `LD_LIBRARY_PATH` within the container has, at the beginning, the additional path for the libraries in the host (as intended). And the rest is the original definition of the variable within the container (as intended).
> ``` bash
...
```
### DEMO: Run a machine learning distributed training on GPUs with tensorflow container
...
This example uses the training procedure described in the tensorflow tutorials for distributed training using Keras: [Distributed training with Keras](https://www.tensorflow.org/tutorials/distribute/keras). The procedure has been included in the python script: `distributedMNIST.py` which, as the name suggests, uses the classical MNIST dataset to train the model to recognise handwritten single digits between 0 and 9. 
...
Here we ask for all the gpus available in a single node (2 in this case). And a single master task is launched. The distribution of work and communication among the GPUS is managed by tensorflow through nvidia tools. But what really concerns to this training is the meaning of the options given to the singularity command:
- `-e` avoids the propagation of any environment variable of the host towards the container (except for those explicilty defined with Singularity syntax). This is the recommended way to execute any Python container in order to avoid contamination of the internal Python environment
- `-B fake_home:$HOME` this is also related to Python. For this application, Python tries to download the MNIST dataset into the user's home directory. As it is not a good practise to bind mount the home directory for containers execution, system adminsitrators have not mounted it by default. And instead of bind mounting it by ourselves, we recommend to use an auxiliary directory (`fake_home` in this case) and bind mount it as `$HOME`. In this way, Python applications will write into this auxiliary directory instead of the real home.
...
Several directories are produced with the training checkpoints, the saved model and, of course, the dataset under the `fake_home`.
...
```output
total 168
-rw-rw----+ 1 espinosa pawsey0001   4139 May 20 13:34 distributedMNIST.py
drwxrws---+ 4 espinosa pawsey0001   4096 May 20 14:09 fake_home
-rw-rw----+ 1 espinosa pawsey0001    514 May 20 14:25 gpu_pawsey.sh
drwxr-s---+ 3 espinosa pawsey0001   4096 May 20 14:26 logs
drwxr-s---+ 2 espinosa pawsey0001   4096 May 20 14:27 training_checkpoints
-rw-rw----+ 1 espinosa pawsey0001 142944 May 20 14:27 slurm-311273.out
drwxr-s---+ 4 espinosa pawsey0001   4096 May 20 14:27 saved_model
```
For this demo, a `typical_output.out` file is also provided, where you can check that both GPUs were used during training. 
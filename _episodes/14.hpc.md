...
Several directories are produced including the `training_checkpoints`, the `saved_model`, the `logs` and, as mentioned above, the dataset and other Python files hidden output under the `fake_home`.

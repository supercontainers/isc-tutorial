...
sed -i -e "s/ntasks=.*/ntasks=$aeg_numberOfSubdomains/" -e "s/ntasks\-per\-node=.*/ntasks\-per\-node=$aeg_numberOfSubdomains/" slurm_pawsey.sh
sed -i "s/NTASKS=.*/NTASKS=\"$aeg_numberOfSubdomains\"/" mpirun.sh
